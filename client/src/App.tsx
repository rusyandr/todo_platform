...
function App() {
  ...
  // (Removed duplicate simple auth-restoration effect.)
  // track whether we've attempted to restore auth (so workspace logic waits)
  ...
  useEffect(() => {
    ...
    // Start auth + initial data load. We only mark authLoaded after initial teams load
    setAuthLoaded(false);
    ...
    api
      ...
      .then(async (res) => {
        ...
        // load teams immediately (independent of loadTeams closure) so workspace can wait
        setTeamsLoading(true);
        try {
        ...
        // also fetch subjects list (best-effort)
        setSubjectsLoading(true);
        try {
  ...
  // Загружаем предметы, когда пользователь авторизовался
  useEffect(() => {
    if (!currentUser) {
      setSubjects([]);
      setSubjectEntities([]);
  ...
  // Open team in a new tab by its join code (URL will be `/${joinCode}`)
  const handleSelectTeam = (teamJoinCode: string) => {
    const url = `${window.location.origin}/${teamJoinCode}`;
    window.open(url, '_blank');
  };

  // If user navigates to `/${CODE}` directly, try to open that team:
  // Handle a path like `/<CODE>` only once on initial load to avoid
  // repeatedly opening the join modal during in-app interactions.
  ...
  useEffect(() => {
    if (!authLoaded) return; // wait until we've attempted auth restore
    ...
    // remember that we've processed the initial path so we don't re-open modals later
    ...
    // not logged in — show join modal (user should login first)
    setIsTeamJoinModalOpen(true);
  }, [authLoaded, currentUser, teams]);

  // If the app was opened directly at `/<CODE>`, render a simplified workspace
  // without the left menu. This avoids duplicating the main UI into the workspace.
  const pathCode = ((): string | null => {
    const source = (initialPathRef.current ?? window.location.pathname) || '/';
    const p = source.slice(1).toUpperCase();
    return p && /^[A-Z0-9]+$/.test(p) ? p : null;
  })();

  if (pathCode && (window.location.pathname !== '/')) {
    // Render minimal workspace view
    return (
      <div className="workspace-shell">
        <header className="workspace-header">
          <div>
            <h1>Рабочая область: {pathCode}</h1>
            <p className="muted">Откройте проект или присоединитесь по коду</p>
          </div>
          <div>
            <button type="button" className="ghost small" onClick={() => { window.location.href = '/'; }}>
              Вернуться
            </button>
          </div>
        </header>
        <main className="workspace-main">
          {/* Wait for auth restoration and team list load to avoid UI flash */}
          {(!authLoaded || (currentUser && teamsLoading)) ? (
            <div className="panel-placeholder">Загрузка…</div>
          ) : currentUser ? (
            (() => {
              const found = teams.find((t) => t.joinCode === pathCode);
              if (found) {
                // if user is a member, fetch and show team details inside workspace
                return (
                  <div className="workspace-team">
                    ...
                    {teamDetails && (
                      <TeamDetails
                        ...
                        onClose={() => { /* workspace keeps open */ }}
                      />
                    )}
                  </div>
                );
              }
              // user is logged in but not yet member
              return (
                <div className="workspace-join">
                  <p>Вы не участник этой команды.</p>
                  <p>Код команды: <strong>{pathCode}</strong></p>
                  <div className="workspace-actions">
                    <button
                      ...
                      onClick={async () => {
                        try {
                          await api.post('/teams/join', { joinCode: pathCode });
                          await Promise.all([loadSubjects(), loadTeams()]);
                          const membership = teams.find((t) => t.joinCode === pathCode);
                          if (membership) {
                            setSelectedTeamId(membership.id);
                          }
                        } catch (err) {
                          console.error(err);
                        }
      ...
      {showDebug && (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <strong>DEBUG</strong>
            <button type="button" className="ghost small" onClick={() => setShowDebug(false)}>Закрыть</button>
          </div>
          <div style={{ marginTop: 8 }}>
            <div>authLoaded: <strong>{String(authLoaded)}</strong></div>
            <div>accessToken: <strong>{localStorage.getItem('accessToken') ? 'present' : 'absent'}</strong></div>
            <div>currentUser: <strong>{currentUser ? `${currentUser.id} ${currentUser.name}` : 'null'}</strong></div>
            <div>teamsLoading: <strong>{String(teamsLoading)}</strong></div>
            <div>teamsCount: <strong>{teams.length}</strong></div>
            <div>pathCode: <strong>{pathCode ?? '-'}</strong></div>
          </div>
        </div>
      )}
